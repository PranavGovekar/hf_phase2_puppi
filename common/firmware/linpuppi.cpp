...
void packed_fwdlinpuppi(const ap_uint<l1ct::PFRegion::BITWIDTH> input_reg, const ap_uint<l1ct::HadCaloObj::BITWIDTH_SLIM> input_calo[NCALO], ap_uint<LINPUPPI_DATA_SIZE_FWD> output[LINPUPPI_NCHANN_FWD_OUT]) {
    #pragma HLS ARRAY_PARTITION variable=input_calo complete
    ...
    #pragma HLS ARRAY_PARTITION variable=caloin complete
    #pragma HLS ARRAY_PARTITION variable=pfselne complete
    region = PFRegion::unpack(input_reg);
    l1pf_pattern_unpack_slim<NCALO,0>(input_calo, caloin);
...
void linpuppi_pack_in(const PFRegion & region, const TkObj track[NTRACK], z0_t pvZ0, const PFNeutralObj pfallne[NALLNEUTRALS], ap_uint<LINPUPPI_DATA_SIZE_IN> input[LINPUPPI_CHS_NCHANN_IN]) {
    ...
    input[0] = region.pack();
    l1pf_pattern_pack_slim<NTRACK, TK_OFFS>(track, input);
    input[PV_OFFS] = 0;
...
void linpuppi_unpack_in(const ap_uint<LINPUPPI_DATA_SIZE_IN> input[LINPUPPI_CHS_NCHANN_IN], PFRegion & region, TkObj track[NTRACK], z0_t & pvZ0, PFNeutralObj pfallne[NALLNEUTRALS]) {
    ...
    l1pf_pattern_unpack_slim<NTRACK, TK_OFFS>(input, track);
...
packed_linpuppi_refobj packed_linpuppi_prepare_track(const ap_uint<TkObj::BITWIDTH_SLIM> & track, const z0_t & pvZ0) {
    #pragma HLS PIPELINE II=1
    #pragma HLS LATENCY min=3
